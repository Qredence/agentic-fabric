...
export function validateWorkflowDefinition(def: unknown): { valid: boolean; errors?: string[] } {
  const validate =
    ajv.getSchema(workflowSchema.$id as string) || ajv.compile(workflowSchema as any);
...
export function checkReferentialIntegrity(def: any): { valid: boolean; errors?: string[] } {
  const errs: string[] = [];
  if (!def || !Array.isArray(def.executors) || !Array.isArray(def.edges))
    ...
    for (const g of def.edgeGroups) {
      if (g.type === 'single') {
        ...
        for (const c of g.cases)
          if (!ids.has(c.target)) errs.push(`Unknown case target: ${c.target}`);
        if (g.default && !ids.has(g.default.target))
          errs.push(`Unknown default target: ${g.default.target}`);
      }
    }
  }
