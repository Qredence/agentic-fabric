...
describe("Pattern visuals consistency", () => {
  it("renders SwitchCase with status and error", () => {
    render(
      ...
      <ReactFlowProvider>
        <SwitchCaseNode id="s1" data={{
        variant: "switch-case",
        handles: { target: true, source: true },
        group: { id: "g1", source: "a", cases: [{ id: "c1", value: "x", target: "b" }], switchExpression: "x", default: { target: "b" } },
        label: "Handoff",
        status: "error",
  ...
  it("renders WorkflowExecutor with nesting indicator", () => {
    render(
      // @ts-ignore test simplified props
      <ReactFlowProvider>
        <WorkflowExecutorNode id="w1" data={{
        variant: "workflow-executor",
        handles: { target: true, source: true },
        executor: { id: "we1", type: "workflow-executor", workflowId: "child", label: "Child Workflow" },
        childCount: 2
      }} />
      </ReactFlowProvider>
    )
