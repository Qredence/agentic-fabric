...
export const ToolApprovalContent = memo(
  ({ className, children, ...props }: ToolApprovalContentProps) => (
    <AlertDescription className={cn('flex items-center gap-2', className)} {...props}>
      {children}
    </AlertDescription>
  ),
);
...
export const ToolApprovalRequest = memo(({ children }: ToolApprovalRequestProps) => {
...
export const ToolApprovalAccepted = memo(({ children }: ToolApprovalAcceptedProps) => {
...
export const ToolApprovalRejected = memo(({ children }: ToolApprovalRejectedProps) => {
  const { approval, state } = useToolApproval();
