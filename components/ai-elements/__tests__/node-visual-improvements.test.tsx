...
import { NodeHeader, NodeContent, NodeFooter } from '@/components/ai-elements/node';
...
describe('Node Visual Improvements', () => {
  const mockData: AgentExecutorNodeData = {
  ...
  describe('Agent Executor Node Visual Enhancements', () => {
    ...
    it('applies enhanced card styling with gradients and shadows', () => {
      renderWithProvider(<AgentExecutorNode id="test" data={mockData} selected={false} />);
    ...
    it('enhances suggestion buttons with better hover effects', () => {
      ...
      const suggestions = screen
        .getAllByRole('button')
        .filter((btn) => btn.className.includes('group/suggestion'));
  ...
  describe('Skeleton Loading States', () => {
    it('renders node skeleton with shimmer animation', () => {
      ...
      expect(skeletonLoaders[0]).toHaveClass(
        'bg-gradient-to-r',
        'from-transparent',
        'via-foreground/10',
        'to-transparent',
