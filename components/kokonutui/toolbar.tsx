...
interface ToolbarProps {
  ...
  onItemSelect?: (id: ToolbarItemId) => void;
  visibleItems?: ToolbarItemId[];
...
export function Toolbar({
  ...
  const DEPRECATED: ToolbarItemId[] = [
    'shapes',
    'layers',
    'frame',
    'properties',
  ...
  const effectiveVisible = visibleItems
    ? visibleItems
    : filterDeprecated
      ? (['select' as ToolbarItemId, 'move'] as ToolbarItemId[])
      : toolbarItems.map((t) => t.id);

  const itemsToRender = toolbarItems.filter((item) => effectiveVisible.includes(item.id));
  ...
  const handleItemClick = (itemId: string) => {
    ...
    if (onItemSelect) {
      try {
        onItemSelect(itemId as ToolbarItemId);
      } catch {}
    }
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {itemsToRender.map((item) => (
            ...
            >
              <item.icon size={16} className={cn(selected === item.id && 'text-white')} />
